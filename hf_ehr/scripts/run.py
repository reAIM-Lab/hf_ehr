...
def main(config: DictConfig) -> None:
    ...
    # print(f"tokenizer_metadata is {tokenizer_metadata}")
    ...
    # Check if resuming from checkpoint. if exsit, will continue training
    ...
    if config.data.tokenizer.name == 'DescTokenizer':
        ...
        print(config.data.tokenizer.path_to_config)
    ...
    #     logger.info(prof.key_averages().table(sort_by="flops", row_limit=20))
    try:
        trainer.fit(
            model, 
            train_dataloaders=dataloaders['train'],
