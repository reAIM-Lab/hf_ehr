...
class NumericalRangeTCE(TokenizerConfigEntry):
    ...
    def to_token(self)-> str:
        # LOINC/1234 || mg/dL || 0.0 - 100.0
        # print( f"numerical {self.code} || {self.tokenization['unit']} || {self.tokenization['range_start']} - {self.tokenization['range_end']}")
        # print( f"numerical_new {self.code.split('//', 1)[0]} || {self.tokenization['unit']} || {self.tokenization['range_start']} - {self.tokenization['range_end']}")
        return f"{self.code.split('//', 1)[0]} || {self.tokenization['unit']} || {self.tokenization['range_start']} - {self.tokenization['range_end']}"
...
class CategoricalTCE(TokenizerConfigEntry):
    ...
    def to_token(self)-> str:
        # LOINC/1234 || category1,category2,category3
        # print(f"categories are {self.code} || {','.join(self.tokenization['categories'])}")
...
# typically do not pass SLURM_JOB_PARTITION as a script argument, it is set default to be gpu
