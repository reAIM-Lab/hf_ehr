...
def load_task_embeddings(args, labels, data_path, device,split):
    ...
    embeddings, labels, ids, times, gflops_per_split = load_meds(labels, data_path, model, tokenizer, device, args,split)

    return embeddings, labels, gflops_per_split, ids, times,
...
def load_meds(data, data_path, model, tokenizer, device, args,split):
    ...
    for file in files:
        ...
        # print(f"file is {Path(file).stem}")
        ...
        if save_path.exists():
            ...
            embeddings, labels, ids, prediction_times,gflops_per_file = get_embeddings(model, tokenizer, events, data, device)
            gflops_per_split.append(gflops_per_file)
    ...
    return batch_embeddings, batch_labels, batch_ids, batch_times, gflops_per_split
...
def get_embeddings(model, tokenizer, subject_data, labels, device):
        ...
        with torch.no_grad():
            ...
            for event in prof.key_averages():
                if hasattr(event, 'flops') and event.flops > 0:
                    gflops_per_batch += event.flops / 1e9  # Convert to GFLOPs

            gflops_per_file.append(gflops_per_batch)
    ...
    return batch_embedding, sorted_labels, sorted_ids, sorted_times, sum(gflops_per_file)
