...
def filter_tokenizer_config(tokenizer_config: List[TokenizerConfigEntry],
    ...
    for entry in tokenizer_config:
        ...
        # print(f"entry is {entry}")  ###entry is CodeTCE(code='MIMIC_IV_Drug/Rapaflo', type='code', description=None, tokenization={}, stats=[])
        # print(f"get stat is {entry.get_stat('count_occurrences', None)}") ###get stat is None

        # Get the count_occurrences stat and check if it exists
        stat = entry.get_stat('count_occurrences', None)
        if stat is None:
            print(f"Missing 'count_occurrences' for token: {entry.code}")
            sys.exit(0)
            # Optionally, you can choose to skip or handle this token differently
            continue

        # Remove tokens with < `min_code_occurrence_count` occurrences in our dataset
        if min_code_occurrence_count is not None and int(stat.count) < min_code_occurrence_count:
            print(f"Token {entry.code} has a count of {stat.count}, which is below the threshold {min_code_occurrence_count}")
            invalid_entries.append(entry.to_token())
            continue
        ...
        #     and int((entry.get_stat('count_occurrences', None)).count) < min_code_occurrence_count
        # ):
        #     # print(f"the count is: {entry.get_stat('count_occurrences', None).count}, {min_code_occurrence_count}")
        #     invalid_entries.append(entry.to_token())
        #     continue
    ...
    if keep_n_max_occurrence_codes is not None:
        print(f"judge {keep_n_max_occurrence_codes is not None}")
        print(f"keep_n_max_occurrence_codes is {keep_n_max_occurrence_codes}")
...
class BaseTokenizer(PreTrainedTokenizer):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        print(f"meta type is {type(self.metadata)}")
        # if isinstance(self.metadata, DictConfig):
        #     self.metadata = OmegaConf.to_container(self.metadata, resolve=True)

        # for key, value in  self.metadata.items():
        #     if isinstance(key, (DictConfig, ListConfig)):
        #         print(f"Found OmegaConf type: {type(key)}) item is {key}")
        #     if isinstance(value, (DictConfig, ListConfig)):
        #         print(f"Found OmegaConf type: {type(value)}) item is {value}")

        if not isinstance(self.metadata, (DictConfig, ListConfig)):
            self.metadata = OmegaConf.create(self.metadata)
        # Now convert the entire config to a native Python container.
        self.metadata = OmegaConf.to_container(self.metadata, resolve=True)
    ...
    # def find_configs(data, path=''):
    #     if isinstance(data, dict):
    #         for key, value in data.items():
    #             new_path = f"{path}.{key}" if path else key
    #             if isinstance(value, (DictConfig, ListConfig)):
    #                 print(f"Found OmegaConf config at key: {new_path} (type: {type(value)})")
    #             # Recursively check the value
    #             find_configs(value, new_path)
    ...
    def get_path_to_tokenizer_version_dir(self) -> str:
        ...
        # print(f"path_to_tokenizer_dir is {path_to_tokenizer_dir}")
        ...
        print(f"path_to_new_folder is {path_to_new_folder}")
    ...
    def get_seq_length_per_patient(self, dataset, n_procs: int = 16, is_force_refresh: bool = False) -> List[int]:
        """
        ...
        if n_procs == 1:
            ...
            with ctx.Pool(processes=n_procs) as pool:
                results: List[List[Tuple[int,int]]] = list(tqdm(
                    pool.imap_unordered(self.get_seq_length, tasks),
                    total=len(tasks),
        ...
        # print(flattened_results[0])
        # print(sorted(flattened_results, key=lambda x: x[0]))
...
class BaseCodeTokenizer(BaseTokenizer):
    ...
    '''
    [Event(code='Ethnicity/Unknown', value=None, unit=None, start=datetime.datetime(2014, 8, 16, 0, 0), end=None, omop_table=None), Event(code='Gender/M', value=None, unit=None, start=datetime.datetime(2014, 8, 16, 0, 0), end=None, omop_table=None)
    '''
    ...
    def __call__(self, 
        ...
        # batch are nested lists, return of sampler. each list is equal length usually
        batch: List[List[str]] = [ self.convert_events_to_tokens(x) for x in batch_of_events]
        # for i in range(len(batch)):
        #     print(f"The shape of {i}th element of batch shape is: {len(batch[i])}")
        ...
        # print(f"after The batch shape is: {batch[0]}")
        if is_truncation_random:
            ...
            # print(f"keys are {tokenized_batch.keys()}")
            # print(tokenized_batch['input_ids'][0])
            # print()
            ...
            for key in tokenized_batch.keys():
                # print(f"The key is {key} tokenized batch shape is: {len(tokenized_batch[key])}")
                # print(f"The key is {key} tokenized batch shape is: {tokenized_batch[key].shape}")
            ...
            try:
                # no further splitting and direct mapping into vocabulary
                # print("directly call __call__")
                tokenized_batch: Dict[str, torch.Tensor] = super().__call__(batch, **kwargs, is_split_into_words=True)
                # print(f"tokenized_batch keys is {tokenized_batch.keys()}")
                # print(f"tokenized_batch is {tokenized_batch['input_ids'][0]}")
...
class CustomTokenizer(BaseCodeTokenizer):
    """
        Settings:
    ...
    def __init__(self,
                 path_to_tokenizer_config: str, 
                 metadata: Optional[Dict[str, Any]] = None,creation=False) -> None:
    ...
    def __call__(self, 
        ...
        """Tokenize a batch of patient timelines, where each timeline is a list of event codes.
            ...
            NOTE: Must set `is_split_into_words=True` b/c we've already pre-tokenized our inputs (i.e. we're passing in a List of tokens, not a string)
        """
        # print("caling call function!")
        if not isinstance(batch_of_events[0], list):
            # List[Event] => List[List[Event]]
            batch_of_events = [ batch_of_events ] # type: ignore
        
        # First, convert all Events => batches of tokens, values and times
        batch_results: List[List[str]] = [ self.convert_events_to_tokens(x) for x in batch_of_events]

        # List[List[strings]]
        codes = [result['codes'] for result in batch_results]
        values = [result['values'] for result in batch_results]
        times = [result['times'] for result in batch_results]
        # batch: List[List[str]] = [ self.convert_events_to_tokens(x) for x in batch_of_events]
        # Second, add special tokens (if applicable)
        if kwargs.get("add_special_tokens", False):
            codes = [ [ self.cls_token, self.bos_token ] + x + [ self.eos_token ] for x in codes ]
        ...
        if is_truncation_random:
            ...
            kwargs.pop('max_length')
            kwargs.pop('truncation')
            tokenized_batch: Dict[str, torch.Tensor] = super().__call__(codes, **kwargs, truncation=None, is_split_into_words=True)

            # Truncate at random positions
            # start_idxs contain a list of start indexes for each patient 

            # for key,value in tokenized_batch.items():
            #     print(f"keys are {key}, values are {value.shape}")
            ...
            # truncate value and times
            truncated_values = []
            truncated_times = []
            for idx, val_list in enumerate(values):
                truncated_values.append(val_list[start_idxs[idx]:start_idxs[idx] + max_length])
            for idx, time_list in enumerate(times):
                truncated_times.append(time_list[start_idxs[idx]:start_idxs[idx] + max_length])
 
            # Pad values and times to the same length, a bit different from other implementation
            if kwargs.get('return_tensors') == 'pt':
                # Pad values and times to the same length
                max_len = max(len(x) for x in truncated_values)
                padded_values = [x + [None] * (max_len - len(x)) for x in truncated_values]
                padded_times = [x + [None] * (max_len - len(x)) for x in truncated_times]
                tokenized_batch['values'] = padded_values
                tokenized_batch['times'] = padded_times
            else:
                tokenized_batch['values'] = truncated_values
                tokenized_batch['times'] = truncated_times
            # max_len = max(len(x) for x in truncated_values)
            # padded_values = [x + [None] * (max_len - len(x)) for x in truncated_values]
            # padded_times = [x + [None] * (max_len - len(x)) for x in truncated_times]
            # if kwargs.get('return_tensors') == 'pt':
            #     padded_values = torch.stack(padded_values)
            #     padded_times = torch.stack(padded_times)
            # tokenized_batch['values'] = truncated_values
            # tokenized_batch['times'] = truncated_times
        ...
        # haven't be revised
            ...
            try:
                # no further splitting and direct mapping into vocabulary
                # print("directly call __call__")
                tokenized_batch: Dict[str, torch.Tensor] = super().__call__(codes, **kwargs, is_split_into_words=True)
                # print(f"tokenized_batch keys is {tokenized_batch.keys()}")
                # print(f"tokenized_batch is {tokenized_batch['input_ids'][0]}")
            except Exception as e:
                print("Exception in BaseCodeTokenizer.__call__():", e)
    ...
    def convert_event_to_token(self, e: Event, **kwargs) -> Optional[str]:
        ...
        return (event_code,event_value,event_time)
    ...
    def convert_events_to_tokens(self, events: List[Event], **kwargs):
        '''
        return: {"codes":list,"values":list,"times":list}
        '''
        tokens = {"codes":[],"values":[],"times":[]} 
        ...
        for e in events:
            ...
            if token is not None:
                code,value,time = token
                tokens["codes"].append(code)
                tokens["values"].append(value)
                tokens['times'].append(time)
        return tokens
    ...
    def convert_events_to_tokens(self, events: List[Event]) -> List[str]:
        ...
        for e in events:
            ...
            if "Visit" in e.code:
                ...
                # you could change it to IATT tokens
                if previous_visit_end is not None:
...
def collate_femr_timelines(batch: List[Tuple[int, List[Event]]],
    ...
    """
        ...
        The input batch is like [[id1,timelines],[id2,timelines]...]
    """

    timelines: List[List[Event]] = [ x[1] for x in batch if len(x[1]) > 0 ] # remove empty timelines
    # count = 0
    # if count <=2:
    # print(f"timelines are {timelines}")
    # sys.exit(0)
    ...
    # tokens["input_ids"] = tokens["input_ids"].bfloat16()
    # tokens["attention_mask"] = tokens["attention_mask"].bfloat16()
    # tokens["labels"] = tokens["labels"].bfloat16()
    return {
