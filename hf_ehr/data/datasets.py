...
class MEDSDataset(BaseDataset):
    ...
    def __init__(self, 
        ...
        self._meds_db = None  # placeholder
    ...
    def _lazy_init_meds_db(self):
        # only open DB if not already opened
        if self._meds_db is None:
            import meds_reader
            self._meds_db = meds_reader.SubjectDatabase(self.path_to_meds_reader_extract, num_threads=1)
    
    def __getitem__(self, idx: int) -> Tuple[int, List[Event]]:
        """Return all event codes for this patient at `idx` in `self.split`.
        """
        if self._meds_db is None:
            self._lazy_init_meds_db()
        ...
        events: List[Event] = [
            ...
            for e in self._meds_db[pid].events
        ]
        return (pid, events)
...
if __name__ == '__main__':
    from hf_ehr.data.tokenization_new import CLMBRTokenizer, DescTokenizer
    import time
