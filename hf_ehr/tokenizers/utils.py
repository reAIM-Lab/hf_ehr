...
from hf_ehr.data.tokenization_new import CookbookTokenizer
from hf_ehr.data.tokenization_new import DescTokenizer
import time
        ...
        for event in femr_db[pid].events:
            if dataset_cls == 'FEMRDataset':
                ...
                if (
                    ...
                    # print(event.code,event.unit,event.numeric_value)
...
def calc_unique_codes(args: Tuple) -> Set[str]:
    ...
    results: Set[str] = set()
    for pid in pids:
        for event in femr_db[pid].events: # events in a certain subject
...
def calc_code_2_occurrence_count(args: Tuple) -> Dict[str, int]:
    ...
    start_time = datetime.datetime.now()
    # if args.tokenizer_name == "cookbook":
    #     tokenizer = CookbookTokenizer(path_to_tokenizer_config, metadata=metadata)
    # elif args.tokenizer_name == "desc":
    #     tokenizer = DescTokenizer(path_to_tokenizer_config, metadata=metadata)
    # no matter cookbook or desc, we can use cookbook tokenizer to tokenize data
    tokenizer = CookbookTokenizer(path_to_tokenizer_config, metadata=metadata,creation=True)
    print(f"Finish | {tokenizer_name} | Time= {datetime.datetime.now() - start_time}s")
    ...
    # Process events
    # this process cause several hours
    ...
    for pid in tqdm(pids, total=len(pids), desc='pids'):
        for event in femr_db[pid].events:
            if dataset_cls == 'FEMRDataset':
                ...
                unit: str = event.unit if event.numeric_value is not None else None
...
def add_unique_codes(path_to_tokenizer_config: str, path_to_extract: str, dataset_cls: str, pids: List[int], path_to_cache_dir: str, **kwargs):
    ...
    print(f"path is {path_to_cache_dir}")
    os.makedirs(path_to_cache_dir, exist_ok=True)
    ...
    save_tokenizer_config_to_path(path_to_tokenizer_config, tokenizer_config, metadata)  # save to 
...
def add_occurrence_count_to_codes(path_to_tokenizer_config: str, path_to_extract: str, dataset_cls: str, pids: List[int], path_to_cache_dir: str, dataset: str = "v8", split: str = "train",tokenizer_name: str = "cookbook", **kwargs):
    ...
    start_time = datetime.datetime.now()
    results = run_helper(calc_code_2_occurrence_count, merge_code_2_occurrence_count, path_to_extract, dataset_cls, pids, path_to_cache_dir,additional_args=(path_to_tokenizer_config,tokenizer_name), **kwargs)
