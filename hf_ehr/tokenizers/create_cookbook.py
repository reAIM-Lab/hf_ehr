...
'''
Usage:
eg:  python create_cookbook.py  --path_to_dataset_config =  .../data/meds_mimic4.yaml  --path_to_tokenizer_config = .../configs/tokenizer/cookbook.yaml --n_procs 5 --chunk_size 10000 --is_force_refresh
'''
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dataset',required=True, type=str,help='specify the dataset to use')
...
def main():
    ...
    # Load dataset config
    # path_to_extract = meds reader, dataset_cls = "MEDS"
    ...
    # # With `n_procs=5`, should take ~XXXX mins
    # # sys.exit(0)
    call_func_with_logging(add_occurrence_count_to_codes, 'add_occurrence_count_to_codes', path_to_tokenizer_config, pids=pids, path_to_cache_dir=args.path_to_cache_dir, dataset=args.dataset, split='train', n_procs=args.n_procs, chunk_size=chunk_size, path_to_extract=path_to_extract, dataset_cls=dataset_cls)
    tokenizer_config, _ = load_tokenizer_config_and_metadata_from_path(path_to_tokenizer_config)
