...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--path_to_database", required=True, type=str, help="Path to MEDS patient database")
    ...
    parser.add_argument("--task_name", required=True, type=str, help="downstream task name")
    parser.add_argument("--output_dir",required=True,type=str,help="output_dir path")
    parser.add_argument("--path_to_model", required=True, type=str, help="Path to model .ckpt")
...
def main():
    ...
    # /data/processed_datasets/processed_datasets/ehr_foundation_data/ohdsi_cumc_deid/ohdsi_cumc_deid_2023q4r3_v3_mapped/task_labels/in_house_phenotypes/phenotype_cohorts_min_obs_2_years
    PATH_TO_MODEL = args.path_to_model
    TASK_NAME = args.task_name
    # MODEL: str = args.path_to_model.split("/")[-3] if args.model_name in [ None, '' ] else args.model_name
    MODEL: str = args.path_to_model
    CKPT: str = get_ckpt_name(PATH_TO_MODEL)
    PATH_TO_LABELED_PATIENTS: str = os.path.join(PATH_TO_LABELS_DIR, TASK_NAME)
    batch_size: int = args.batch_size if args.batch_size not in [None, ''] else 16
    device: str = args.device

    model_signature: str = f'{MODEL}_{CKPT}'
    PATH_TO_OUTPUT_FILE: str = os.path.join(args.output_dir, args.task_name)
    ...
    run_name = f"chunk_strat={CHUNK_STRAT},max_length={max_length}_{config.data.tokenizer.name}"  + "_tokenized_timelines"
