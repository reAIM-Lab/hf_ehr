...
def load_tokenizer_from_config(config):
    ...
    from hf_ehr.data.tokenization_new import CookbookTokenizer, CLMBRTokenizer, DescTokenizer, CEHRTokenizer
    ...
    if name == 'CLMBRTokenizer':
        ...
        tokenizer_metadata = {
            ...
            'is_remap_numerical_codes_to_quantiles': tokenizer__is_remap_numerical_codes,
            'min_code_occurrence_count': tokenizer__min_code_count
        }
        tokenizer = CookbookTokenizer(path_to_config,metadata=tokenizer_metadata)
...
def get_dataset_info_from_config_yaml(path_to_config_yaml: str) -> Tuple[str, str]:
    ...
    if 'data' in config_dataset:
        ...
        dataset_cls: str = config_dataset['dataset']['name']
        if dataset_cls == 'FEMRDataset':
            path_to_extract: str = config_dataset['dataset']['path_to_femr_extract']
        elif dataset_cls == 'MEDSDataset':
            path_to_extract: str = config_dataset['dataset']['path_to_meds_reader_extract']
        else:
            raise NotImplementedError(f'Dataset `{dataset_cls}` not yet implemented in `get_dataset_info_from_config_yaml()` function.')
